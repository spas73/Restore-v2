...
export default function ProductForm({
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...product,
        file: undefined, // non popoliamo il file se siamo in modalità modifica
      });
    }
  ...
  const onSubmit = async (data: CreateProductSchema) => {
    try {
      // Validazione manuale solo se si sta creando un nuovo prodotto
      if (!product && !watchFile) {
        setError("file", { message: "A file must be uploaded" });
        return;
      }
